var RealGrids = (function () {
    ...
    var win = window;
    var doc = document;
    var REQUIRED_VERSION = "1.1.18.2527";
    var TRACE = false;
...
//    });
    ...
    */
    ...
    DataProvider.prototype = {
        ...
        //Validation 을 체크할 컬럼과 call back함수를 담을 배열
		addedValidation : [],
		//Validation을 추가한다.
		addDataValidation : function(a,o,c,g) {
			var s = this;  // 현재 개체를 담는다. Properti내의 함수에서 parent method를 call하기 위해서
			
			if (g===undefined) g= "grdMain";
			
			this.addedValidation.push({
					column : a, //컬럼
					text   : __grids[g].getColumnHeader(__grids[g].columnByField(a)).text,
					option : o, //Option empty:공백 체크 fn:call back
					callBack : c===undefined || c==""?function(){alert(this.text+" 는 필수 입력 항목입니다.");}:c // Call Back 이Null일경우는 기본 Alert을 띄운다.
			});			
		},
		//Validation을 체크한다. empty일경우는 기본 call back 호출
		getDataValidation: function(a) {
			//Validation 항목이 없으면 Validation  Pass
		    if  (this.addedValidation.length==0) return true;
		    
			for(var o in this.addedValidation)
			{
				if (!isNaN(o))
				{
					//Option 이 공백일경우의 처리
					if (this.addedValidation[o].option==="empty")
					{
						if (a[this.addedValidation[o].column]==="" || a[this.addedValidation[o].column]== null || a[this.addedValidation[o].column]=="null") 
						{
							this.addedValidation[o].callBack();
							return false;
						}
					}
					//공백이 아닐경우 Custom callback 호출
					else
					{
						if (!this.addedValidation[o].callBack({ field : this.addedValidation[o].column, value : a[this.addedValidation[o].column],object : a })) return false;
					}
				}
			}
    ...
    */
    ...
    LocalDataProvider.prototype = _extend(new DataProvider(), {
        ...
        /*
         * Row가 없을경우 검색결과 없음 표시
         */
        isEmpty : function(rows) {
        	if (rows.length==0) 
        	{
        		// 이전에 생성된 개체가 있을경우 지움
        		if ($("#"+'m_'+this.dom.id)!==undefined) $("#"+'m_'+this.dom.id).remove();

        		/*
        		 * 메세지 DIV 삽입
        		 */
        	    $('<div>')
        	      .attr('style', "position:absolute; text-align: center; display:block; height:25px;width:"+$("#"+this.dom.id).width()+"px;color:orange;font-size: 1.3em;padding-top:"+this.dom.getHeader().height+"px;")
        	      .attr('id','m_'+this.dom.id)
        	      .html("검색결과가 없습니다.")
        	      .insertBefore($('#'+this.dom.id))
        	      .animate({opacity: 1.0}, 3000)     //5초후 메세지 삭제
        	    ...
        	    /*
        	     * 삽입된 메세지와 그리드 사이의 간격 계산 
        	     */
        	    var _top  =$("#"+this.dom.id).offset().top-$("#m_"+this.dom.id).offset().top;
        		
        	    /*메세지 위치를 그리드 헤더 밑으로 이동
        	     * 그리드 헤더의 높이가 0일경우 25로 고정
        	     */
        		$("#m_"+this.dom.id).css({"padding-top" : (this.dom.getHeader().height==0?25:this.dom.getHeader().height)+_top})
